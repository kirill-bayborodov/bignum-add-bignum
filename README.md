...
[![C/ASM CI](https://github.com/kirill-bayborodov/bignum-add-bignum/actions/workflows/ci.yml/badge.svg)](https://github.com/kirill-bayborodov/bignum-add-bignum/actions/workflows/ci.yml)
[![GitHub release](https://img.shields.io/github/v/release/kirill-bayborodov/bignum-add-bignum?label=release)](https://github.com/kirill-bayborodov/bignum-add-bignum/releases/latest)

`bignum-add-bignum` is a standalone C11/x86-64 YASM module that adds two fixed-capacity unsigned `bignum_t` values. The production path is an x86-64 YASM implementation conforming to the System V AMD64 ABI. The operation validates pointers, capacities, overlap, and representability before writing a normalized sum transactionally.
...
The required components are included as Git submodules under `libs/`. The frozen template Makefile builds source dependencies recursively and links the pinned benchmark-framework distribution.

| Component | Expected location | Purpose |
|---|---|---|
| `bignum-core` | `libs/bignum-core` | Defines `bignum_t`, `BIGNUM_CAPACITY`, and common primitives |
| `bignum-cmp` | `libs/bignum-cmp` | Compares bignum values in tests and dependency code |
| `bignum-init` | `libs/bignum-init` | Initializes bignum objects |
| `bignum-init-from-array` | `libs/bignum-init-from-array` | Initializes bignum objects from words |
| `bignum-normalize` | `libs/bignum-normalize` | Normalizes high zero words |
...
```bash
git clone --recurse-submodules https://github.com/kirill-bayborodov/bignum-add-bignum.git
cd bignum-add-bignum
```
...
Before the first build, initialize all submodules with `git submodule update --init --recursive`.

## Features

- **Production ASM path:** x86-64 YASM implementation for the System V AMD64 ABI.
- **Explicit API status:** the public API exposes `bignum_add_bignum_status_t` rather than reusing a generic core status type.
- **Carry-chain implementation:** the ASM path uses native x86-64 carry propagation and the C11 path is a readable reference implementation.
- **Bounds protection:** invalid lengths, overlap, and unrepresentable sums return explicit errors without changing the destination.
- **Transactional operation:** successful calls leave both input values unchanged; failed calls leave the destination unchanged.
...
- **Bignum domain adapter:** `benchmarks/adapter/` maps generic transport fields to bignum-specific `add` and operand-length semantics.
...
The public API is declared in `include/bignum_add_bignum.h`:

```c
typedef enum {
    BIGNUM_ADD_BIGNUM_SUCCESS = 0,
    BIGNUM_ADD_BIGNUM_ERROR_NULL_PTR = -1,
    BIGNUM_ADD_BIGNUM_ERROR_CAPACITY_EXCEEDED = -2,
    BIGNUM_ADD_BIGNUM_ERROR_BUFFER_OVERLAP = -3,
    BIGNUM_ADD_BIGNUM_ERROR_OVERFLOW = -4
...
|---|---|---|
| Any required pointer is `NULL` | `BIGNUM_ADD_BIGNUM_ERROR_NULL_PTR` | Destination is unchanged |
| An operand length exceeds `BIGNUM_CAPACITY` | `BIGNUM_ADD_BIGNUM_ERROR_CAPACITY_EXCEEDED` | Destination is unchanged |
| `result` partially overlaps `a` or `b` | `BIGNUM_ADD_BIGNUM_ERROR_BUFFER_OVERLAP` | Destination is unchanged |
| The sum requires more than `BIGNUM_CAPACITY` words | `BIGNUM_ADD_BIGNUM_ERROR_OVERFLOW` | Destination is unchanged |
| Valid operands and representable sum | `BIGNUM_ADD_BIGNUM_SUCCESS` | `result` receives normalized `a + b` |

The operation permits exact in-place updates (`result == a` or `result == b`) and is reentrant for independent objects. It does not mutate either input. Concurrent writers still require external synchronization.
...
bignum_add_bignum_status_t add_values(
    bignum_t *result, const bignum_t *a, const bignum_t *b)
{
    return bignum_add_bignum(result, a, b);
}
```
...
| File | Scope |
|---|---|
| `tests/test_bignum_add_bignum.c` | Deterministic API, contract, and boundary tests |
| `tests/test_bignum_add_bignum_extra.c` | Extended state, preservation, and boundary checks |
| `tests/test_bignum_add_bignum_mt.c` | Concurrent independent-object checks |
| `tests/test_bignum_add_bignum_runner.c` | Distribution integration smoke test |
| `tests/benchmark_adapter/test_bignum_add_bignum_benchmark_adapter.c` | C11 transport mapping, validation, deterministic initialization, operation, and checksum tests |
...
```text
benchmarks/bench_bignum_add_bignum.c
benchmarks/bench_bignum_add_bignum_mt.c
```
...
```text
benchmark=bignum_add_bignum_st ... elapsed_seconds=<seconds> ns_per_call=<nanoseconds>
Benchmark finished.
```

The MT runner uses `benchmark=bignum_add_bignum_mt`. The trailing marker is the success condition checked by the Makefile; it must remain after the machine-readable line.

| Mode | Input pattern | Purpose |
|---|---|---|
| `all_zero` | Every input `bignum_t` is zero; operand lengths is zero | Measures the zero-value fast path |
| `all_nonzero` | Inputs are populated through `BIGNUM_CAPACITY` words with nonzero top words | Measures the normal nonzero addition path |
| `mixed` | Alternating zero and nonzero input rows | Measures a mixed workload and branch behavior |

### Single-thread CLI

```text
bin/bench_bignum_add_bignum \
  [--data-mode all_zero|all_nonzero|mixed] \
  [--input-kind zero|nonzero|mixed] \
  [--operation-kind add|add|add|add|mixed] \
...
| `BENCH_OPERATION_KIND` | `add` | `add`, `add`, `add`, `add`, or `mixed` bignum operation transport value |
...
```bash
./bin/bench_bignum_add_bignum \
  --input-kind nonzero --operation-kind add --size-profile half \
...
```text
bin/bench_bignum_add_bignum_mt \
...
The reusable benchmark implementation is the public `libs/benchmark-framework` Git submodule pinned to `v1.0.0`. The project-local ST and MT sources call its `benchmark-core` lifecycle through `benchmarks/adapter/bignum_add_bignum_benchmark_adapter.c`. The adapter validates bignum vocabulary, constructs deterministic `bignum_t` records, chooses representable operand lengths, and maps `bignum_add_bignum_status_t` to the named framework callback status.
...
`bench_matrix` invokes the pinned C11 `bench_matrix` and `benchmark_stats` tools directly, without Python or hardware PMU events. The default `benchmarks/profiles/bignum_add_bignum_full.json` covers zero/nonzero addition paths, one/quarter/half/variable operand lengths, and safe near-capacity cases. `benchmarks/profiles/bignum_add_bignum_standard.json` is the shorter addition-specific smoke manifest and can be selected through `BENCH_MATRIX_PROFILE`. Each JSON manifest has a companion how-to document with its exact vocabulary and baseline workflow.
