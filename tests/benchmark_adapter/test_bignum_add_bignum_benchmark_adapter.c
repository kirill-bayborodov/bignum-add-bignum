/**
 * @file test_bignum_add_bignum_benchmark_adapter.c
 * @brief Deterministic tests for the bignum_add_bignum benchmark adapter.
...
static void test_validation(void)
{
    ...
    assert(bignum_add_bignum_benchmark_validate_workload(&workload) ==
        BIGNUM_ADD_BIGNUM_BENCHMARK_STATUS_SUCCESS);
    invalid.operation_kind = "xor";
    assert(bignum_add_bignum_benchmark_validate_workload(&invalid) ==
        BIGNUM_ADD_BIGNUM_BENCHMARK_STATUS_INVALID_PROFILE);
    assert(bignum_add_bignum_benchmark_validate_workload(NULL) ==
        BIGNUM_ADD_BIGNUM_BENCHMARK_STATUS_NULL_ARGUMENT);
}
...
static void test_callbacks(void)
{
    ...
    assert(bignum_add_bignum_benchmark_adapter_init(NULL) ==
        BIGNUM_ADD_BIGNUM_BENCHMARK_STATUS_NULL_ARGUMENT);
    assert(bignum_add_bignum_benchmark_adapter_init(&adapter) ==
        BIGNUM_ADD_BIGNUM_BENCHMARK_STATUS_SUCCESS);
...
int main(void)
{
    ...
    test_legacy_modes();
    test_callbacks();
    puts("bignum_add_bignum benchmark adapter tests: OK");
    return EXIT_SUCCESS;
}
